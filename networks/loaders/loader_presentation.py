...
class ImageFolder(data.Dataset):
	...
	def __getitem__(self, index):
		...
		current_img = image_file.pixel_array
		...
		current_GT = cv.imread(GT_path, cv.IMREAD_GRAYSCALE)
		...
		if not self.is_multiorgan:
			...
			current_GT = current_GT/np.max(current_GT)
			current_GT = current_GT > 0.5
			current_GT = current_GT.astype(np.float32)	
		else:
			GT = self.mask_to_class(GT)
			current_GT = self.mask_to_class(current_GT)
		...
		current_GT = np.expand_dims(current_GT,axis=-1)
		augmented={}
		if self.mode == "train": 
		...
		GT = to_tensor(GT)
		current_img = to_tensor(current_img)
		current_GT = to_tensor(current_GT)
		
		
				
		return image, GT, current_img, current_GT

	def __len__(self):
		"""Returns the total number of font files."""
		return len(self.image_paths)

def get_loader(image_path, image_size, batch_size, num_workers=2, mode='train',is_multiorgan=True, augmentation_prob=0.8, classes = None, shuffle = True):
...
path='C:\\Users\\ek779475\\Desktop\\PRO_pCT_CGFL\\multiclass_imbalanced\\train'
...
for (image,mask, curr, curr_mask) in dataload:
	# transforms = T.Compose([T.ToPILImage()])
	image = image.data.cpu().detach().numpy().squeeze()
	mask = mask.data.cpu().detach().numpy().squeeze()
	curr = curr.data.cpu().detach().numpy().squeeze()
	curr_mask = curr_mask.data.cpu().detach().numpy().squeeze()
	mask = np.ma.masked_where(mask == 0, mask)
	curr_mask = np.ma.masked_where(curr_mask == 0, curr_mask)
	...
	ax2.imshow(curr_mask, cmap="jet",alpha=0.5)
